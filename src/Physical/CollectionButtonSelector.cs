...
namespace Appalachia.Simulation.Physical
{
    public abstract class CollectionButtonSelector
    {
        ...
        public static CollectionButtonSelector<PhysicsMaterials, PhysicMaterialWrapper>
            CreatePhysicMaterialSelector(Action<PhysicMaterialWrapper> select)
        {
            return new(PhysicsMaterials.instance, select, ColorPrefs.Instance
               .PhysicMaterialSelectorButton, ColorPrefs.Instance.PhysicMaterialSelectorColorDrop);
        }

        public static CollectionButtonSelector<PhysicsMaterials, PhysicMaterialWrapper>
            CreatePhysicMaterialSelector(
                Action<PhysicMaterialWrapper> select,
                PREF<Color> buttonColor)
        {
            return new(PhysicsMaterials.instance, select, buttonColor, ColorPrefs.Instance
               .PhysicMaterialSelectorColorDrop);
        }

        public static CollectionButtonSelector<DensityMetadataCollection, DensityMetadata>
            CreateDensityMetadataSelector(Action<DensityMetadata> select)
        {
            return new(DensityMetadataCollection.instance, select, ColorPrefs.Instance
               .DensitySelectorButton, ColorPrefs.Instance.DensitySelectorColorDrop);
        }

        public static CollectionButtonSelector<TC, TV> CreateGenericSelector<TC, TV>(
            TC instance,
            Action<TV> select)
        ...
        where TValue : AppalachiaScriptableObject<TValue>, ICategorizable
    {
        ...
        public CollectionButtonSelector(
            T instance,
