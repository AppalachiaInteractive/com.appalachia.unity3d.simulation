...
namespace Appalachia.Simulation.Trees.UI.Graph
{
    ...
    {
        public static TreeGraphLayoutResult LayoutTree(TreeGraphSettings settings, TreeGraphNode root)
        {
        ...
        {
            var pxSlop = siblings[siblingIndex].xRelativeToLeftSibling -
                         settings.nodeWidth -
            ...
            if (pxSlop > 0f)
            {
                ...
                siblings[siblingIndex].xRelativeToLeftSibling -=
                    ((siblingIndex - responsibleIndex - 1f) * pxSlop) / (siblingIndex - responsibleIndex);
            }
        }
            ...
            TreeGraphNode node)
        {
            ...
            foreach (var child in node.children)
            {
                ...
                width = Mathf.Max(
                    width,
                    (currentMinXRelativeToBoundingBox + child.subtreeWidth) - child.minXRelativeToBoundingBox
                );
        ...
        private static void CenterOverChildren(TreeGraphSettings settings, TreeGraphNode node)
        {
            ...
            node.minXRelativeToBoundingBox = ((pxLeftChild + pxRightChild) - settings.nodeWidth) / 2f;
            ...
            TreeGraphNode node)
        {
            foreach (var child in node.children)
            {
                child.minXRelativeToParent = child.minXRelativeToBoundingBox - node.minXRelativeToBoundingBox;
            }
        }
            ...
            List<int> responsibleNodeIndices)
        {
            ...
            if (siblingIndex == 0)
            {
                ...
                {
                    xsRelativeToBoundingBox.Add(pxRelativeToRoot + sibling.minXRelativeToBoundingBox);
            ...
            sibling.xRelativeToLeftSibling = (newXRelativeToBoundingBox - xsRelativeToBoundingBox.First()) +
                                             settings.nodeWidth +
            ...
            for (var i = xsRelativeToBoundingBox.Count; i < sibling.maxXRelativeToRoot.Count; i++)
            {
                xsRelativeToBoundingBox.Add(
                    sibling.maxXRelativeToRoot[i] + newXRelativeToBoundingBox + horizontalBufferPixels
                );
