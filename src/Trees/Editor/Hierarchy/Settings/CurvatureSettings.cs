...
namespace Appalachia.Simulation.Trees.Hierarchy.Settings
{
    ...
    public class CurvatureSettings : AgeOverrideResponsiveSettings, ICloneable<CurvatureSettings>
    {
        ...
        [PropertyTooltip(
            "Use the curve to adjust how the branches are curl back towards their parent."
        )]
        [PropertyRange(0f, 1f), TreeProperty]
        ...
        [PropertyTooltip("Scale of the noise around the branch, lower values will give a more wobbly look, while higher values gives a more stochastic look.")]
        [PropertyRange(0f, 8.0f),TreeProperty, ShowIf(nameof(showNoiseSettings))]
        [OnValueChanged(nameof(GeometrySettingsChanged), true )]
        public floatTree noiseScaleU = TreeProperty.New(0.9f);

        [PropertyTooltip("Scale of the noise along the branch, lower values will give a more wobbly look, while higher values gives a more stochastic look.")]
        [PropertyRange(0f, 8.0f),TreeProperty, ShowIf(nameof(showNoiseSettings))]
        [OnValueChanged(nameof(GeometrySettingsChanged), true )]
