...
{
    [Serializable]
    public class CollisionMeshCollection : AppalachiaSimpleBase
    {
        ...
        {
            collisionMesh = new Mesh { name = ZString.Format("{0}{1}", meshName, meshSuffix) };
            decompositionMeshNamePart = ZString.Format("{0}{1}", meshName, meshSuffix_wood);
        }
        ...
        public bool IsCorrectlyApplied(GameObject go, TreeGlobalSettings globals)
        {
            ...
            {
                Context.Log.Warn("Prefab update required: Wrong collider parent name.");
                return false;
            }
            ...
            if (tc.Length > 0)
            {
                Context.Log.Warn("Prefab update required: Extra mesh collider.");
                return false;
            }
            ...
            if (childCount != requiredChildCount)
            {
                Context.Log.Warn("Prefab update required: Wrong mesh collider count.");
                return false;
            }

            {
                ...
                {
                    Context.Log.Warn("Prefab update required: Wrong mesh collider child name.");
                    return false;
                }
                ...
                if (models.Count == 1)
                {
                    if (collisionMesh.vertexCount <= decompositionMeshes[0].vertexCount)
                    {
                        ...
                        {
                            Context.Log.Warn("Prefab update required: Collision mesh not correct.");
                            return false;
                        }
                    }
                    else
                    ...
                    {
                        Context.Log.Warn(
                            "Prefab update required: Collision mesh cooking settings not correct."
                        );
                        return false;
                    }
                    ...
                    {
                        Context.Log.Warn("Prefab update required: Mesh collider not convex.");
                        return false;
                    }

                    if (colliders[0].sharedMaterial != globals.woodMaterial)
                    {
                        Context.Log.Warn("Prefab update required: Wrong material on wood sphere.");
                        return false;
                    }
                }
                else
                {
                    for (var i = 0; i < colliders.Length; i++)
                    {
                        ...
                        {
                            Context.Log.Warn("Prefab update required: Wrong material on wood collider.");
                            return false;
                        }
                    }
                }
            ...
            if (requireFungus)
            {
                ...
                if (colliders.Length != models.Count)
                {
                    Context.Log.Warn("Prefab update required: Wrong fruit sphere count.");
                    return false;
                }
                ...
                for (var i = 0; i < colliders.Length; i++)
                {
                    ...
                    {
                        Context.Log.Warn("Prefab update required: Bad center on fruit sphere.");
                        return false;
                    }

                    if (Math.Abs(colliders[i].radius - models[i].radius) > float.Epsilon)
                    {
                        Context.Log.Warn("Prefab update required: Bad radius on fruit sphere.");
                        return false;
                    }

                    if (colliders[i].sharedMaterial != globals.mushroomMaterial)
                    {
                        Context.Log.Warn("Prefab update required: Wrong material on fungus sphere.");
                        return false;
                    }
                }
            }
            ...
            if (requireFruit)
            {
                ...
                for (var i = 0; i < colliders.Length; i++)
                {
                    ...
                    {
                        Context.Log.Warn("Prefab update required: Wrong material on fruit sphere.");
                        return false;
                    }
                }
            }
        ...
        public void SaveMeshes(Func<Mesh, Mesh> saveFunc)
        {
            ...
            if (decompositionMeshes != null)
            {
                for (var i = 0; i < decompositionMeshes.Count; i++)
                {
                    ...
                    decompositionMeshes[i].name = ZString.Format("{0}{1}", decompositionMeshNamePart, i);
