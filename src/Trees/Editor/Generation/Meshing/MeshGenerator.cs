...
namespace Appalachia.Simulation.Trees.Generation.Meshing
{
    ...
    public static class MeshGenerator
    {
            ...
            int groupingScale)
        {
            using (BUILD_TIME.MESH_GEN.RecalculateNormals.Auto())
            {
                ...
                for (var submeshIndex = 0; submeshIndex < mesh.subMeshCount; submeshIndex++)
                {
                    ...
                    {
                        var i1 = triangles[triI];
                        var i2 = triangles[triI + 1];
                        var i3 = triangles[triI + 2];
            ...
            Bounds vertexBounds)
        {
            using (BUILD_TIME.MESH_GEN.CreateMeshFromData.Auto())
            {
                ...
                for (var i = 0; i < output.vertices.Count; i++)
                {
                    ...
                    if (realGeometry)
                    {
                        shapeLookup[vertex.shapeID]
                           .geometry[output.lodLevel]
                           .actualVertices.Add(actualVertexIndex);
                    }
                ...
                if ((mArray == null) || (mArray.Length == 0) || forceNoSubmesh)
                {
                    ...
                    {
                        var materialElement =
                            mArray[mArray.Length - 1].GetMaterialElementForLOD(output.lodLevel);
                    ...
                    for (var j = 0; j < output.triangles.Count; j++)
                    {
                        ...
                        if (realGeometry)
                        {
                            shapeLookup[triangle.shapeID]
                               .geometry[output.lodLevel]
                               .actualTriangles.Add(triangleIndexOffset / 3);
                        }
                    ...
                    foreach (var material in mArray)
                    {
                        ...
                        for (var j = 0; j < output.triangles.Count; j++)
                        {
                                ...
                                (material.MaterialContext == MaterialContext.TiledOutputMaterial))
                            {
                                ...
                                if (m.inputMaterialID == triangle.inputMaterialID)
                                {
                                    ...
                                    if (realGeometry)
                                    {
                                        shapeLookup[triangle.shapeID]
                                           .geometry[output.lodLevel]
                                           .actualTriangles.Add(fullTriangleIndexOffset / 3);
                                    }
                                     ...
                                     (material.MaterialContext == MaterialContext.AtlasOutputMaterial))
                            {
                                ...
                                var aim =
                                    inputMaterials.GetByMaterialID(triangle.inputMaterialID) as
                                        AtlasInputMaterial;
        ...
        private struct vkey : IEquatable<vkey>
        {
            ...
            #region IEquatable<vkey> Members
