...
namespace Appalachia.Simulation.Trees.Generation.Texturing.Operations
{
    public static class TextureCombiner
    {
            ...
            using (BUILD_TIME.TEX_COMBR.DrawAtlasTextures.Auto())
            {
                ...
                try
                {
                    ...
                    foreach (var inputMaterial in inputMaterials)
                    {
                        ...
                        var node = new TextureAtlasNode(
                            nodeSize, textureSize)
                        {
                ...
                var textures = DrawTextures(
                    ...
                    inputMaterials, debugCombinationOutputs, true);
            ...
            bool debugCombinationOutputs)
        {
            using (BUILD_TIME.TEX_COMBR.DrawTiledTextures.Auto())
            {
                ...
                var textures = DrawTextures(
                    ...
                    new[] {inputMaterial}, debugCombinationOutputs, false);
            ...
            bool atlas)
        {
            using (BUILD_TIME.TEX_COMBR.DrawTextures.Auto())
            {
                ...
                try
                {
                    ...
                    {
                        var combinerMaterial = new Material(GSR.instance.textureCombiner);
                        ...
                        for (var i = 0; i < 4; i++)
                        {
                            anyChannelsPresent = anyChannelsPresent |
                                inputMaterialChannelLookupCollection.Any(ud => ud.ContainsKey(outputProfile[i]));
                        }
                        ...
                        var newTextureName = $"{output.TexturePrefix}_{outputProfile.fileNameSuffix}";
                        ...
                        var flipperMaterial = new Material(GSR.instance.textureFlipper);
                        ...
                        */
                        ...
                        {
                            var path = AssetDatabaseManager.GetAssetPath(output.GetMaterialElementByIndex(0).asset);

                            if (!string.IsNullOrWhiteSpace(path))
                            {
                                combinerMats.Add(
                                    new Tuple<string, Material>(
                                        path,
                                        combinerMaterial
                                    )
                                );
            ...
            InputMaterial inputMaterial)
        {
            ...
            var rect = inputMaterial.GetRect(
                materialSize,  outputTextureSize
            );
        ...
        public static class Properties
        {
            ...
            {
                return string.Format(TEX_FORMAT, (TextureChannel) channel);
            }

            public static string Channel(int channel)
            {
                return string.Format(TEX_CHANNEL_FORMAT, (TextureChannel) channel);
            }

            public static string Invert(int channel)
            {
                return string.Format(TEX_INVERT_FORMAT, (TextureChannel) channel);
            }

            public static string PackStyle(int channel)
            {
                return string.Format(TEX_PACKSTYLE_FORMAT, (TextureChannel) channel);
            }

            public static string FixedValue(int channel)
            {
                return string.Format(TEX_FIXEDVALUE_FORMAT, (TextureChannel) channel);
            }

            public static string Mode(int channel)
            {
                return string.Format(TEX_MODE_FORMAT, (TextureChannel) channel);
            }
