...
namespace Appalachia.Simulation.Trees.Settings
{
    ...
    public class TreeSettings : TypeBasedSettings<TreeSettings>
    {
        ...
        public void Check()
        {
            if (ao == null) ao = new AmbientOcclusionSettings(ResponsiveSettingsType.Tree);
            if (transmission == null) transmission = new TransmissionSettings(ResponsiveSettingsType.Tree);
            if (wind == null) wind = new WindSettings(ResponsiveSettingsType.Tree);
            if (lod == null) lod = new LevelOfDetailSettingsCollection(ResponsiveSettingsType.Tree);
            if (material == null) material = new MaterialSettings(ResponsiveSettingsType.Tree);
            if (texture == null) texture = new TextureSettings(ResponsiveSettingsType.Tree);
            if (mesh == null) mesh = new MeshSettings(ResponsiveSettingsType.Tree);
            if (collision == null) collision = new CollisionSettings(ResponsiveSettingsType.Tree);
            if (variants == null) variants = new TreeVariantSettings();
        }
