...
namespace Appalachia.Simulation.Trees.Settings.Log
{
    ...
    [Title(
        "Settings",
        "Control the generation of all logs.",
    ...
    public class LogSettings : TypeBasedSettings<LogSettings>
    {
        ...
        public void Check()
        {
            if (vertex == null) vertex = new VertexSettings(ResponsiveSettingsType.Log);
            if (lod == null) lod = new LevelOfDetailSettingsCollection(ResponsiveSettingsType.Log);
            if (material == null) material = new LogMaterialSettings(ResponsiveSettingsType.Log);
            if (texture == null) texture = new TextureSettings(ResponsiveSettingsType.Log);
            if (mesh == null) mesh = new MeshSettings(ResponsiveSettingsType.Log);
            if (collision == null) collision = new CollisionSettings(ResponsiveSettingsType.Log);
        }
    }
}