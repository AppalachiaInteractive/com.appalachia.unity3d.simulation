    ...
    public static class BranchBuildManager
    {
        ...
        internal static void Update()
        {
            using (_PRF_Update.Auto())
            {
                try
                {
                    if ((CTX != null) && (CTX.progressTracker != null) && (CTX.progressTracker.timeSinceBuildComplete > 600) &&
        ...
        public static IEnumerator ExecuteAllBuildsEnumerator(QualityMode quality, Action buildAction,
                                                             string searchString)
        {
        ...
        private static IEnumerator ExecutePendingBuild(bool automatic = false)
        {
            ...
            //{
                ...
                try
                {
                    ...
                    if ((branchData.buildState == BuildState.Disabled) || (branchData.requestLevel == BuildRequestLevel.None))
                    {
                        EditorCoroutineUtility.StopCoroutine(_coroutine);
                        completed = true;
                        yield break; // ========================================================== BREAK
                    ...
                    branchData.materials.inputMaterialCache.SetDefaultMaterials(branchData.settings.material.defaultMaterials);
                ...
                if (branchData.buildRequest.ShouldBuild(BuildCategory.MaterialGeneration, requestLevel))
                {
                    ...
                    if (branchData.materials.RequiresUpdate(branchData.settings.material, out _))
                    {
                        ...
                        */
                        ...
                        {
                            MaterialPropertyManager.SetMaterialNonProportionalAreas(branchData.materials.inputMaterialCache);
                        }
                        ...
                        var combinedInputMaterials = branchData.materials.inputMaterialCache.atlasInputMaterials
                            .Concat<InputMaterial>(branchData.materials.inputMaterialCache.tiledInputMaterials)
                            .ToArray();
                        ...
                        textureOutputCollection.Add(branchData.materials.outputMaterialCache.atlasOutputMaterial, outputTextures);


                        branchData.progressTracker.Update(BuildCategory.MaterialGeneration);

                        foreach (var tiledMaterial in branchData.materials.outputMaterialCache.tiledOutputMaterials)
                        {
                            var match = branchData.materials.inputMaterialCache.tiledInputMaterials.FirstOrDefault(
                            ...
                            var ot = TextureCombiner.DrawTiledTextures(
                                match, tiledMaterial, adjustedTextuerSize,
        ...
        private static void GenerateDistribution(BuildRequestLevel requestLevel, BranchDataContainer branchData)
        {
            using (BUILD_TIME.TREE_BUILD_MGR.GenerateIndividualStageShapes.Auto())
            {                
                TreeProperty.SetActiveGenerationAge(AgeType.Mature);

                var regenerateAOSampling = branchData.buildRequest.ShouldBuild(BuildCategory.AmbientOcclusion, requestLevel) || (branchData.samplePoints == null);
                ...
                if (branchData.buildRequest.distribution == requestLevel)
                {
                    ...
                    ShapeGenerationManager.GenerateShapes(branchData.branch.shapes, branchData.branch.hierarchies);
                ...
                if (regenerateAOSampling)
                {
                    ...
                    branchData.samplePoints = AmbientOcclusionGenerator.GenerateAOSamplePoints(branchData.branch);
                }
            }
        }
            ...
            ref bool completed)
        {
            using (BUILD_TIME.TREE_BUILD_MGR.GenerateBranchMaterial.Auto())
            {
                ...
                foreach (var snapshot in branchData.snapshots)
                {
                    ...
                    foreach (var outputProfile in snapshot.branchOutputMaterial.GetOutputTextureProfiles(true))
                    {
        ...
        {
            Vector4 minimums = Vector4.one;
            Vector4 maximums = Vector4.zero;
            ...
            for (int i = 0; i < pixels.Length; i++)
            {
                ...
                if (r) { pixel.r = (byte)(255 * (rRange == 0.0f ? 0.0f : (pixel.r - minimums.x) / rRange)); }
                if (g) { pixel.g = (byte)(255 * (gRange == 0.0f ? 0.0f : (pixel.g - minimums.y) / gRange)); }
                if (b) { pixel.b = (byte)(255 * (bRange == 0.0f ? 0.0f : (pixel.b - minimums.z) / bRange)); }
                if (a) { pixel.a = (byte)(255 * (aRange == 0.0f ? 0.0f : (pixel.a - minimums.w) / aRange)); }
