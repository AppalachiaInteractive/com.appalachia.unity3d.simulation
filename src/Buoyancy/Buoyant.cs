...
namespace Appalachia.Simulation.Buoyancy
{
    ...
    public class Buoyant : InternalInstancedMonoBehaviour
    {
        ...
        [NonSerialized, ShowInInspector, FoldoutGroup("State")] public float wetness;
        [NonSerialized, ShowInInspector, FoldoutGroup("State")] public float submersionWetness;
        ...
        [FoldoutGroup("State"), SmartLabel, NonSerialized, ShowInInspector]
        ...
        [FoldoutGroup("State"), SmartLabel(Postfix = true), Sirenix.OdinInspector.ReadOnly, ShowInInspector]
        public bool submerged => percentageSubmerged > 0;

        [FoldoutGroup("State"), SmartLabel(Postfix = true), Sirenix.OdinInspector.ReadOnly, NonSerialized, ShowInInspector]
        public bool resting;

        [FoldoutGroup("State"), SmartLabel, Sirenix.OdinInspector.ReadOnly, PropertyRange(0f, 1f), NonSerialized, ShowInInspector]
        public float percentageSubmerged;
        
        [FoldoutGroup("State"), SmartLabel, ShowInInspector]
        public double3Average averagedTorque;

        [FormerlySerializedAs("data")]
        [FoldoutGroup("Setup"), SmartLabel, InlineEditor]
        public BuoyancyData buoyancyData;

        [FoldoutGroup("Setup"), SmartLabel, InlineEditor, ShowInInspector, NonSerialized]
        ...
        [FoldoutGroup("Setup"), SmartLabel, Sirenix.OdinInspector.ReadOnly]
        ...
        public void Initialize()
        {
            using (_PRF_Initialize.Auto())
            {
                ...
                if (buoyancyData == null)
                {
                    ...
                    UnityEditor.AssetDatabase.TryGetGUIDAndLocalFileIdentifier(mesh, out var key, out long _);
                    
                    buoyancyData = BuoyancyDataCollection.instance.GetOrLoadOrCreateNew(key, $"{mesh.name}_{key}");
                ...
                var persistence = lookupInstance.GetOrLoadOrCreateNew(buoyancyData.name, buoyancyData.name);
                
                var renderers = this.FilterComponentsFromChildren<MeshRenderer>().RunFilter();
                var colliders = this.FilterComponentsFromChildren<Collider>().ActiveOnly().NoTriggers().RunFilter();

                var shouldRestore = persistence.ShouldRestore(buoyancyData.voxelResolution, buoyancyData.voxelPopulationStyle, colliders, renderers);
        ...
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs));
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_Initialize = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".Initialize");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_PositionDataUpdate = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".PositionDataUpdate");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_CompletePhysical = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".CompletePhysical");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_Synchronize = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".Synchronize");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_JobHandleComplete = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".JobHandleComplete");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_ResetCenterOfMass = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".ResetCenterOfMass");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_WaterUpdateSamples = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".WaterUpdateSamples");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_CalculateForcesJob = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".CalculateForcesJob");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_CalculateSubmersionJob = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".CalculateSubmersionJob");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_InitializeParameters = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".InitializeParameters");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_MaterialVariables = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".MaterialVariables");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_UpdatePhysical = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".UpdatePhysical");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_AggregateForceAndTorqueJob = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".AggregateForceAndTorqueJob");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_ResetForceAndTorqueJob = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".ResetForceAndTorqueJob");
        private static readonly ProfilerMarker _PRF_ScheduleBuoyancyJobs_CombinedDependencies = new ProfilerMarker(_PRF_PFX + nameof(ScheduleBuoyancyJobs) + ".CombinedDependencies");
        ...
        public void ScheduleBuoyancyJobs(Water water, float deltaTime)
        {
            using (_PRF_ScheduleBuoyancyJobs.Auto())
            {
                try
                {
                    if ((buoyancyData.buoyancyType != BuoyancyType.Physical) && (buoyancyData.buoyancyType != BuoyancyType.PhysicalVoxel))
                    {
                        return;
                    }
                    ...
                    using (_PRF_ScheduleBuoyancyJobs_InitializeParameters.Auto())
                    {
                        ...
                        localToWorldMatrix = (float4x4) _transform.localToWorldMatrix;
                        ...
                        var archimedesForceMagnitude = waterDensityMetadata.densityKGPerCubicMeter * Mathf.Abs(gravity.y) * voxels.worldVolume;
                    ...
                    using (_PRF_ScheduleBuoyancyJobs_MaterialVariables.Auto())
                    {
                        ...
                        var wetSpeed = wetter ? buoyancyData.wetnessAccumulationSpeed : buoyancyData.wetnessDispersalSpeed;
                        var wetnessTarget = wetter ? 1.0f : 0.0f;

                        var submergeder = subPct > .75f;
                        var submersionSpeed = submergeder ? buoyancyData.submergednessAccumulationSpeed : buoyancyData.submergednessDispersalSpeed;
                    ...
                    using (_PRF_ScheduleBuoyancyJobs_AggregateForceAndTorqueJob.Auto())
                    {
                        handle3b1 = new BuoyancyForceJobs.AggregateForceAndTorqueJob()
                        {
                            ...
                            viscousWaterResistanceForce = voxels.objectData.viscousWaterResistanceForce.GetParallel(),
        ...
        [ButtonGroup, EnableIf(nameof(_canDirectRegister))]
        public void DirectRegister()
        {
            var waters = Object.FindObjectsOfType<Water>();
            for (var i = 0; i < waters.Length; i++)
            {
                var water = waters[i];

                Bounds voxelBounds = buoyancyData.voxelPopulationStyle == VoxelPopulationStyle.Colliders
                    ? this.FilterComponentsFromChildren<Collider>().ActiveOnly().NoTriggers().RunFilter().GetEncompassingBounds()
                    : this.FilterComponentsFromChildren<MeshRenderer>().ActiveOnly().RunFilter().GetEncompassingBounds();
        ...
        private static readonly ProfilerMarker _PRF_UpdateDrag = new ProfilerMarker(_PRF_PFX + nameof(UpdateDrag));
        public void UpdateDrag(float submergedAmount)
        {
            using (_PRF_UpdateDrag.Auto())
            {
                ...
                body.drag = originalBodyData.drag + (originalBodyData.drag * percentageSubmerged * waterPhysicsCoefficients.data.additionalDrag);
                body.angularDrag = originalBodyData.angularDrag + (percentageSubmerged * waterPhysicsCoefficients.data.additionalAngularDrag);
            }
        }
        ...
        /// </summary>
        [BurstCompile]
        public static float ResistanceCoefficient(WaterPhysicsCoefficentData metadata, float velocity, float length)
        {
            //Reynolds number
        ...
        private void OnDrawGizmos()
        {
            using (_PRF_OnDrawGizmos.Auto())
            {
                ...
                if (!_buoyancyGizmoSettings.drawGizmos || !_buoyancyGizmoSettings.drawSelectedGizmos)
                {
                    return;
                }
                ...
                {
                    SmartHandles.DrawLine(position, position + (cumulativeForce/body.mass), ColorPrefs.Instance.Buoyancy_CumulativeForce.v);

                    
                    SmartHandles.DrawLine(position, position + (cumulativeTorque/body.mass), ColorPrefs.Instance.Buoyancy_CumulativeTorque.v);
                ...
                if (_buoyancyGizmoSettings.drawCumulativeWaterLevelGizmos)
                {
                    ...
                    SmartHandles.DrawLine(position, waterPosition, ColorPrefs.Instance.Buoyancy_CumulativeToSurface.v);
                }
            }
        }
        ...
        private void OnDrawGizmosSelected()
        {
            using (_PRF_OnDrawGizmosSelected.Auto())
            {
                ...
                if (_voxelGizmoSettings == null)
                {
                    ...
                    _voxelGizmoSettings = lookup.GetOrLoadOrCreateNew(VoxelDataGizmoStyle.Buoyancy, nameof(VoxelDataGizmoStyle.Buoyancy));
                }
                ...
                var drawSelectedSubmersionPositions = _buoyancyGizmoSettings.drawSelectedSubmersionPositions;
                ...
                if (drawSelectedForcePositions)
                {
                    ...
                    for (var i = 0; i < length; i++)
                    {
                        ...
                        var forceGizmoSize = gizmoForceBaseSize + (gizmoForceBaseSize * gizmoForceFlexSize * math.length(buoyancyVoxel.force));
                ...
                if (drawSelectedSubmersionPositions)
                {
                    ...
                    for (var i = 0; i < length; i++)
                    {
                        ...
                        var submersionGizmoSize = gizmoSubmersionBaseSize + (gizmoSubmersionFlexSize * buoyancyVoxel.submersion.value);
                ...
                var drawSelected_cumulatv_ForceLines = _buoyancyGizmoSettings.drawSelectedForceLines;
                var drawSelected_hydrosta_ForceLines = _buoyancyGizmoSettings.drawSelectedHydrostaticForceLines;
                var drawSelected_viscosWR_ForceLines = _buoyancyGizmoSettings.drawSelectedViscousWaterResistanceForceLines;
                var drawSelected_presrDrg_ForceLines = _buoyancyGizmoSettings.drawSelectedPressureDragForceLines;
                var drawSelected_airResis_ForceLines = _buoyancyGizmoSettings.drawSelectedAirResistanceForceLines;
                var drawSelected_windResi_ForceLines = _buoyancyGizmoSettings.drawSelectedWindResistanceForceLines;
                var drawSelected_waveDrft_ForceLines = _buoyancyGizmoSettings.drawSelectedWaveDriftingForceLines;
                var drawSelected_slamming_ForceLines = _buoyancyGizmoSettings.drawSelectedSlammingForceLines;

                var drawAnyForceLine = (buoyancyData.buoyancyType == BuoyancyType.Physical || buoyancyData.buoyancyType == BuoyancyType.PhysicalVoxel) &&
                ...
                if (drawAnyForceLine)
                {
                    var lineScale = Vector3.one * _buoyancyGizmoSettings.drawSelectedForceLinesScale;
                    var lineOffset = float3c.forward_right * _buoyancyGizmoSettings.drawSelectedForceLinesOffset;
                    ...
                    for (var i = 0; i < length; i++)
                    {
                        ...
                        if (drawSelected_cumulatv_ForceLines && _gizmo_cumulatv_ForceLines == null) _gizmo_cumulatv_ForceLines = new Vector3[length * 2];
                        if (drawSelected_hydrosta_ForceLines && _gizmo_hydrosta_ForceLines == null) _gizmo_hydrosta_ForceLines = new Vector3[length * 2];
                        if (drawSelected_viscosWR_ForceLines && _gizmo_viscosWR_ForceLines == null) _gizmo_viscosWR_ForceLines = new Vector3[length * 2];
                        if (drawSelected_presrDrg_ForceLines && _gizmo_presrDrg_ForceLines == null) _gizmo_presrDrg_ForceLines = new Vector3[length * 2];
                        if (drawSelected_airResis_ForceLines && _gizmo_airResis_ForceLines == null) _gizmo_airResis_ForceLines = new Vector3[length * 2];
                        if (drawSelected_windResi_ForceLines && _gizmo_windResi_ForceLines == null) _gizmo_windResi_ForceLines = new Vector3[length * 2];
                        if (drawSelected_waveDrft_ForceLines && _gizmo_waveDrft_ForceLines == null) _gizmo_waveDrft_ForceLines = new Vector3[length * 2];
                        if (drawSelected_slamming_ForceLines && _gizmo_slamming_ForceLines == null) _gizmo_slamming_ForceLines = new Vector3[length * 2];
                        ...
                        if (drawSelected_cumulatv_ForceLines) _gizmo_cumulatv_ForceLines[indexA] = cumulatv_base;
                        if (drawSelected_hydrosta_ForceLines) _gizmo_hydrosta_ForceLines[indexA] = hydrosta_base;
                        if (drawSelected_viscosWR_ForceLines) _gizmo_viscosWR_ForceLines[indexA] = viscosWR_base;
                        if (drawSelected_presrDrg_ForceLines) _gizmo_presrDrg_ForceLines[indexA] = presrDrg_base;
                        if (drawSelected_airResis_ForceLines) _gizmo_airResis_ForceLines[indexA] = airResis_base;
                        if (drawSelected_windResi_ForceLines) _gizmo_windResi_ForceLines[indexA] = windResi_base;
                        if (drawSelected_waveDrft_ForceLines) _gizmo_waveDrft_ForceLines[indexA] = waveDrft_base;
                        if (drawSelected_slamming_ForceLines) _gizmo_slamming_ForceLines[indexA] = slamming_base;
                        
                        var indexB = indexA + 1;

                        if (drawSelected_cumulatv_ForceLines) _gizmo_cumulatv_ForceLines[indexB] = cumulatv_base + (lineScale * (buoyancyVoxel.force / body.mass));
                        if (drawSelected_hydrosta_ForceLines) _gizmo_hydrosta_ForceLines[indexB] = hydrosta_base + (lineScale * (buoyancyVoxel.hydrostaticForce / body.mass));
                        if (drawSelected_viscosWR_ForceLines) _gizmo_viscosWR_ForceLines[indexB] = viscosWR_base + (lineScale * (buoyancyVoxel.viscousWaterResistanceForce / body.mass));
                        if (drawSelected_presrDrg_ForceLines) _gizmo_presrDrg_ForceLines[indexB] = presrDrg_base + (lineScale * (buoyancyVoxel.pressureDragForce / body.mass));
                        if (drawSelected_airResis_ForceLines) _gizmo_airResis_ForceLines[indexB] = airResis_base + (lineScale * (buoyancyVoxel.airResistanceForce / body.mass));
                        if (drawSelected_windResi_ForceLines) _gizmo_windResi_ForceLines[indexB] = windResi_base + (lineScale * (buoyancyVoxel.windResistanceForce / body.mass));
                        if (drawSelected_waveDrft_ForceLines) _gizmo_waveDrft_ForceLines[indexB] = waveDrft_base + (lineScale * (buoyancyVoxel.waveDriftingForce / body.mass));
                        if (drawSelected_slamming_ForceLines) _gizmo_slamming_ForceLines[indexB] = slamming_base + (lineScale * (buoyancyVoxel.slammingForce / body.mass));
                    }
